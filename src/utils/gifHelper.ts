...
const getWorkerUrl = async (): Promise<string> => {
  ...
  try {
    // Try to load from local public folder first
    ...
    console.warn("Failed to load local worker script, attempting CDN fallback", error);
    try {
      ...
      console.error("Failed to load worker script from CDN", cdnError);
      // Last resort: return the path and hope the browser can load it directly
